...
//   id: string;
//   entries: TimelineEntry[];
//   hl: (text: string) => React.ReactNode;
// }) {
//   return (
...
//       {caption && <p className="text-sm font-semibold text-foreground mb-1">{caption}</p>}
...
// // ── Section renderer (unchanged logic, extracted to function) ─────────────────
...
//     case 'p':
//       return <p id={id} className="text-muted-foreground leading-relaxed mb-4">{hl(content as string)}</p>;
//     case 'list': {
...
//   const { manifest, manifestStatus, chunks, sections, loadChunk, loadUntilSection } = useDocumentChunks(locale);
...
//     // Estimate varies by section type  tables/timelines are taller
...
//   // ── Sentinel: load next chunk when user scrolls near the end ──────────────
...
//         // Find the next un-loaded chunk
//         const nextIdle = chunks.findIndex((c) => c.status === 'idle');
//         if (nextIdle !== -1) loadChunk(nextIdle);
//       },
//       { rootMargin: '400px' } // start loading 400 px before hitting the bottom
//     );
...
//   // First ensure the chunk containing this section is loaded
//   loadUntilSection(sectionId).then(() => {
//     // Wait a tick for React to render the newly loaded sections
//     setTimeout(() => {
//       const sectionIndex = sections.findIndex((s) => s.id === sectionId);
//       if (sectionIndex !== -1) {
//         virtualizer.scrollToIndex(sectionIndex, { align: 'center', behavior: 'smooth' });
//       }
//       // Extra delay for virtualiser to position the row, then scroll to exact mark
//       setTimeout(() => {
...
//   // ── onMatchFound: no-op (search is now index-driven, not DOM-driven) ───────
//   useEffect(() => {
//     onMatchFound([]);
//   }, [onMatchFound]);
...
//         This document has been translated by AI. Some inaccuracies may be present.
//       </p>
//     </div>
//     )}
...
//       {/* Sentinel triggers loading of the next chunk */}
//       <div ref={sentinelRef} className="h-1" aria-hidden />

//       {/* Loading indicator while chunks are being fetched */}
